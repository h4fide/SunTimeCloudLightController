...
const char *ssid = "Dithered";  
const char *password = "errorcdiv";

//const char *ssid = "ZTE_2.4G_PGD3NG";  
//const char *password = "GC6QYxm6";
...
const int BUFFER_SIZE = 6144;  // Increased buffer size for HTML template
char buffer[BUFFER_SIZE];      // Global buffer for HTML generation
...
const int TIME_RISE_OFFSET_MINUTES = 0;     // Base offset after sunrise
const int TIME_SET_OFFSET_MINUTES = 0;      // Base offset before sunset
const int TIME_OFFSET_MONITORING = 30;      // Minutes before sunrise/sunset to start monitoring
const int MAX_MONITORING_RETRIES = 3;       // Maximum number of retries for cloud monitoring
const int RETRY_DELAY_MS = 300000;          // 5 minutes delay between retries

// Change these from const to regular variables so they can be modified
int TIME_RISE_OFFSET_ADDITIONAL = 0;  // Dynamic offset for cloudy conditions at sunrise
int TIME_SET_OFFSET_ADDITIONAL = 0;   // Dynamic offset for cloudy conditions at sunset
...
// Time Configuration
const int TIMEZONE_OFFSET = 0;     // GMT+0 for Morocco
const int DAYLIGHT_OFFSET = 3600;  // 1 hour in seconds
const float LATITUDE = 34.231111;    // Morocco latitude
const float LONGITUDE = -3.9816676;   // Morocco longitude

// Relay State Configuration
int relayOn = LOW;   // Active low configuration
int relayOff = HIGH; // Inactive state
...
void updateLocalTime() {
    ...
    while (!timeClient.update()) {
        ESP.wdtFeed();  // Feed watchdog during potentially long operation
        if (millis() - startTime > 10000) break; // Timeout after 10 seconds
        delay(100);
    }
...
bool isWithinMonitoringWindow(int currentHour, int currentMinute) {
    ...
    // Check if within TIME_OFFSET_MONITORING minutes before sunrise or sunset
...
bool monitorCloudConditions(bool isSunrise) {
    if (monitoring_retry_count >= MAX_MONITORING_RETRIES) {
        // Reset monitoring state and proceed with default behavior
        monitoring_retry_count = 0;
        if (isSunrise) {
            monitoring_sunrise = false;
            monitoring_sunset = true;
            TIME_RISE_OFFSET_ADDITIONAL = 0;  // Reset additional offset
            return false;
        } else {
            monitoring_sunset = false;
            monitoring_sunrise = true;
            TIME_SET_OFFSET_ADDITIONAL = 0;   // Reset additional offset
            return true;
        }
    }
    ...
    if (cloudCoverage < 0) {
        // Failed to get cloud coverage data
        monitoring_retry_count++;
        delay(RETRY_DELAY_MS);
        return monitorCloudConditions(isSunrise); // Recursive retry
    }
    
    monitoring_retry_count = 0; // Reset retry counter on successful reading
    
    if (cloudCoverage > CLOUD_COVERAGE_THRESHOLD) {
        if (isSunrise) {
            // Increase sunrise offset
            TIME_RISE_OFFSET_ADDITIONAL += 30;
            return false; // Keep light on
        } else {
            // Trigger early sunset
            monitoring_sunset = false;
            monitoring_sunrise = true;
            TIME_SET_OFFSET_ADDITIONAL = 0;  // Reset sunset offset
    ...
    if (isSunrise) {
        monitoring_sunrise = false;
        monitoring_sunset = true;
        TIME_RISE_OFFSET_ADDITIONAL = 0;  // Reset sunrise offset
        return false; // Turn light off
    } else {
        monitoring_sunset = false;
        monitoring_sunrise = true;
        TIME_SET_OFFSET_ADDITIONAL = 0;   // Reset sunset offset
        return true; // Proceed with normal sunset timing
    }
}
...
String formatTime(double minutesFromMidnight, bool isSunrise) {
    ...
    // Apply appropriate offset based on sunrise/sunset
    ...
    // Apply additional offset from cloud monitoring
...
void updateSunriseSunsetTime() {
    ...
    // Parse sunrise time with specific sunrise offset
    ...
    // Parse sunset time with specific sunset offset
...
void handleToggle() {
  ...
  // Update status LED to match relay state
...
// Handle Cloud Monitoring
void handleCloudMonitoring() {
    if (!server.authenticate(http_username, http_password)) {
        return server.requestAuthentication();
    }

    // Get fresh cloud coverage data
    float newCloudCoverage = getCloudCoverage();
    
    if (newCloudCoverage >= 0) {  // Valid reading
        currentCloudCoverage = newCloudCoverage;
        
        // Check if we should activate based on cloud coverage
...
void setup() {
  ...
  ESP.wdtEnable(WDTO_8S);  // Enable with longer timeout
...
void loop() {
    ...
    if (wifiEnabled) {
        ...
        // Only disable WiFi if we're not in monitoring state
    ...
    // Light control based on current conditions
    ...
    // Special status LED behavior for cloud-triggered activation
    if (cloudTriggeredActivation) {
        digitalWrite(STATUS_LED_PIN, (millis() / 500) % 2); // Blink for cloud trigger
    } else {
